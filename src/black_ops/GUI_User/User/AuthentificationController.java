...
import black_ops.GUI.LiveStreamingController;
...
//                }
        ...
        try{
            ...
            if(rs.next()){
                ...
                String active_client = rs.getString("Pseaudo_Cl");
                if(active_client.equals("admin")){
                    JOptionPane.showMessageDialog(null, "welcome admin");
                }
                else{
                    CommandefController.idcli = rs.getInt(1);
                    LiveStreamingController.idclcom = active_client;
                    System.out.println(CommandefController.idcli);
                    ChatController.client_name = active_client;
                    Video_uploade_userController.client_name = active_client;
                    Video_list_userController.client_name = active_client;
                    JOptionPane.showMessageDialog(null, "bienvenu "+active_client+" !");
                    switchChat(event);
                }
            }else
...
//                saveSystem(selectedFile, )
    ...
    private void switchChat(ActionEvent event) throws IOException{
           try {
            ...
            btn_image_up.getScene().setRoot(root);
